...
import { getGoogleCalendarAgenda, getGoogleCalendarStatus } from "@/lib/google-calendar";
...
export async function getClinicViewModel(options = {}) {
  ...
  let calendar = Array.isArray(options.calendarOverride) ? options.calendarOverride : clinicData.calendar;
  let calendarMeta = null;

  if (options.syncGoogleAgenda) {
    const agenda = await getGoogleCalendarAgenda(calendar);

    calendar = agenda.calendar;
    calendarMeta = agenda.meta;
  }
