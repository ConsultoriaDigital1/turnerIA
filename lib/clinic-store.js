...
async function initializeDatabase(pool) {
  ...
  try {
    ...
    await client.query("CREATE INDEX IF NOT EXISTS patients_doctor_id_idx ON patients (doctor_id)");
    await client.query("CREATE INDEX IF NOT EXISTS patients_name_idx ON patients (name)");

    const doctorCountResult = await client.query("SELECT COUNT(*)::int AS count FROM doctors");
    const patientCountResult = await client.query("SELECT COUNT(*)::int AS count FROM patients");
...
async function seedDatabase(client) {
  ...
  for (const doctor of seedDashboard.doctors) {
    await client.query(
      `
        INSERT INTO doctors (id, name, specialty, room, shift, notes)
        VALUES ($1, $2, $3, $4, $5, $6)
    ...
    for (const insurance of normalizeInsuranceList(doctor.insurances)) {
      await client.query(
        `
          INSERT INTO doctor_insurances (doctor_id, insurance)
          VALUES ($1, $2)
  ...
  for (const patient of seedDashboard.patients) {
    ...
    await client.query(
      `
        INSERT INTO patients (id, name, age, insurance, doctor_id, phone, last_visit, next_appointment, status)
...
async function getDoctorsFromDatabase() {
  ...
  const result = await pool.query(`
    ...
    FROM doctors d
    LEFT JOIN doctor_insurances di ON di.doctor_id = d.id
