...
async function readPatientsFromTable(client, tableName) {
  ...
  return result.rows.map((row) => ({
    ...
    lastVisit: row.last_visit ? String(row.last_visit).slice(0, 10) : sanitizeText(row.lastVisit),
    nextAppointment: sanitizeText(row.next_appointment || row.nextAppointment),
  ...
  for (const patient of patients) {
    ...
    await client.query(
      ...
      [
        ...
        sanitizeText(patient.lastVisit),
        sanitizeText(patient.nextAppointment),
...
export async function createDoctor(input) {
  ...
  try {
    ...
    await client.query(
      `
        INSERT INTO ${TABLES.doctors} (id, name, specialty, room, shift, plan, notes)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
      `,
      [doctorId, doctor.name, doctor.specialty, doctor.room, doctor.shift, doctor.plan, doctor.notes]
    );

    await replaceDoctorInsurances(client, doctorId, doctor.insurances);
    await client.query("COMMIT");
  } catch (error) {
    await rollbackQuietly(client);
    throw error;
  ...
  try {
    ...
    const result = await client.query(
      `
        ...
        SET name = $2, specialty = $3, room = $4, shift = $5, plan = $6, notes = $7
        WHERE id = $1
        RETURNING id
      `,
      [normalizedDoctorId, doctor.name, doctor.specialty, doctor.room, doctor.shift, doctor.plan, doctor.notes]
    );
    ...
    await replaceDoctorInsurances(client, normalizedDoctorId, doctor.insurances);
    await client.query("COMMIT");
