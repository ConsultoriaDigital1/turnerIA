...
function normalizePatientInput(input) {
  ...
  const nextAppointment = input?.nextAppointment ? normalizeNextAppointment(input.nextAppointment) : "";
...
export async function updatePatient(patientId, input) {
  ...
  const explicitNextAppointment = 'nextAppointment' in (input || {});
  const patient = normalizePatientInput(input);
    ...
    const result = await client.query(
      `
        ...
        SET
          ...
          next_appointment = CASE WHEN $9 IS NULL THEN next_appointment ELSE $9::text END,
      ...
      [
        ...
        patient.phone,
        patient.lastVisit,
        explicitNextAppointment ? patient.nextAppointment : null,
        patient.status
      ]
    );
    ...
    const updated = await client.query(
      `
        SELECT p.id, p.name, p.age, p.insurance, p.plan, p.doctor_id, p.phone,
               TO_CHAR(p.last_visit, 'YYYY-MM-DD') AS "lastVisit",
               p.next_appointment AS "nextAppointment", p.status,
